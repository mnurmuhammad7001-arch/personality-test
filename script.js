...
const questions = [
  ...
  { q: "3. Идеальный вечер пятницы — какой он для вас?",
    a:[
      ...
      "Громкая вечеринка с друзьями, танцы до упаду!"
    ], scores:[1,2,3] 
  },
...
function showResult() {
  ...
  let description; // локальная переменная

  if (totalScore >= 0 && totalScore <= 15) {
    type = "Интроверт"; // глобальная переменная
  ...
  } else if (totalScore >= 26 && totalScore <= 39) { // пример верхнего предела
    type = "Экстраверт";
    description = "Экстраверт – человек, который направляет свою энергию во вне, более комфортно чувствующий себя в окружении людей, чем одному, и набирающий силы от взаимодействия с людьми.";
  } else {
    type = "Не определено"; // на случай некорректного totalScore
    description = "Результат не попал в ожидаемые диапазоны.";
  }
